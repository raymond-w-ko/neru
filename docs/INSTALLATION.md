...
> [!NOTE]
> This is only required if you're using `nix`, you're using the `neru-source` package and nixpkgs is not on golang `1.26.3` yet.

```nix
package = pkgs.neru-source.overrideAttrs (_: {
  postPatch = ''
     substituteInPlace go.mod \
       --replace-fail "go 1.26.3" "go 1.25.5"
